/**
 * Description - This code provides the functionality to do Gigabit file uploads using Node.js and Express.
 *               The client application uploads a Gigabit sized file to the Node.js backend in chunks, 
 *               and each chunk is saved by the Node.js backend as a separate file. 
 ...
 *               
 *               The Node.js backend also provides the ability to resume a file upload.
 *               
 *               Once all of the data hase been received the client sends the Node.js backend a 
 *               mergeall command and the Node.js backend will merge all of the file chunks into a single file.
 *               
    ...
    if (request.headers['celerft-encoded'] === 'base64') {
        ...
        request.on('data', function (chunk) {
            ...
            fs.outputFile(response.locals.localfilepath, fileData, function (err) {
                ...
                response.status(201).send(response.locals.localfilepath);
                response.end();
...
app.post('*/api/CelerFTFileUpload/UploadChunk/FormData*', function (request, response) {
    ...
    form.on('end', function (fields, files) {
        ...
        fs.move(temp_path , response.locals.localfilepath, {clobber:true}, function (err) {
...
// Request to resume the file uploads
app.get('*/api/CelerFTFileUpload/GetChunkInfo*', function (request, response) {
    ...
    if (request.method == 'GET') { 
        ...
        var filename = localFilePath + '/' + baseFilename + '.' + request.param('chunkNumber').toString().padLeft('0', 16) + extension + ".tmp";

        fs.stat(filename, function (error, stats) {

            if (error && error.errno === 34) {

                response.status(404).send(filename);
                //response.end();
                console.log(error);
                return;
            }

            console.log('File chunk found ' + filename);
...
app.get('*/api/CelerFTFileUpload/GetFileInfo*', function (request, response) {
    ...
    if (request.method == 'GET') {
        ...
        fs.readdir(localFilePath, function (error, fileslist) {
            ...
            if (error) {
                ...
                response.status(500).send('Error retrieving the file count');
                //response.end();
                console.log(error);
                return;
            }
            ...
            if ((fileslist.length) == request.param('numberOfChunks')) {
                ...
                outputFile.on('finish', function () {
                    ...
                    fs.move(filename, newfilename , {clobber:true}, function (err) {
                        if (err) {
                            console.log(err);
                            response.status(500).send(err);
                            //runGC();
                            ...
                            fs.remove(localFilePath, function (err) {
                                ...
                                // Send back a sucessful response with the file name
                                response.status(201).send('Sucessfully merged file ' + filename);
...
app.get('*/api/CelerFTFileUpload/GetChecksum*', function (request, response) {
    ...
    if (request.method == 'GET') {
        ...
        hashstream.on('end', function () {
            ...
            response.status(200).send(md5results.toUpperCase());
            response.end();
